	...
	if (errorDBAnden === false) {
		if ((selector) && (verifAnden)) {
			if( ($('.salida#hora').html() == 'Sin datos'))
			{
				...
				$('main#en-servicio').css('display','none');
				$('main#anden-en-servicio').css('display','block');
			}
		};
	};
}
...
function proximaSalida(numRamal, anden) {
	if ((selector) && (verifAnden)) {
		...
		$.get("proxima-salida.php?ramal="+numRamal+"&anden="+anden+"&cartel="+cartel ,function(datosAndenOrigen,status){					
			...
			if (arr.length > 0) {
				...
				if (arr[0]=='ok') {
					...
					if (arr[3] != '1') {
						...
						if (arr[3]=='2') {
							$('.tipo-servicio#salida').html('Servicio rápido'); 
							...
							$('.tipo-servicio#salida').html('Servicio s-rápido'); 
							$('.servicio-especial#salida').css('display','block');
							...
							$('.tipo-servicio#salida').html('Servicio especial'); 
							$('.servicio-especial#salida').css('display','block')
						...
						$('.tipo-servicio#salida').html('Servicio normal'); 
